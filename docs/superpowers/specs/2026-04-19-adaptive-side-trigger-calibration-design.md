...
- **必須 (test 失敗)**：commit 数 ≥ 16（旧 capture の p20/p80 実測 17 に対する回帰保護）
- **目標 (warning のみ)**：commit 数 ≥ 19（新 capture の p20/p80 実測 19/20）
...
- FSM 閾値緩和（`PULL_ENTER = 0.72`, `RELEASE = 0.7`）は今回採用しない。p20/p80 が false positive 0 で 19/20 まで改善したため、閾値変更は別 simulation で扱う
- cooldown / dwell 緩和は今回採用しない。今回の sweep は calibration だけで改善することを確認したため、FSM 時間軸の変更は Phase 2 の独立候補とする
