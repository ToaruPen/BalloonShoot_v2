...
ring buffer から導出した raw extrema を以下の順で clamp：

```ts
clampedPulled = max(pulledLowerBound, p10)                    // default lowerBound = 0
clampedOpen   = min(openUpperBound,   p90)                    // default upperBound = 1.2
// 両者の最小スパンを保証（formula 分母 observedSpan が 0 にならないように）
if (clampedOpen - clampedPulled < pulledOpenMinSpan) {
  // 中点を保ち、両端を均等に押し広げる
  midpoint = (clampedPulled + clampedOpen) / 2
  clampedPulled = max(pulledLowerBound, midpoint - pulledOpenMinSpan / 2)
  clampedOpen   = min(openUpperBound,   midpoint + pulledOpenMinSpan / 2)
}
```
...
- `HandLandmarkSet` に `middleMcp: Point3D` と `pinkyMcp: Point3D` を追加
- `HAND_LANDMARK_INDEX` に `middleMcp: 9, pinkyMcp: 17` を追加
- 既存の filter 配線、telemetry serialization、テストフィクスチャを連動更新

これらは既存処理に影響を与えない pure-add（既存コードはこれらのフィールドを参照していない）。
...
|---|---|---|---|---|
| good 品質 + hand 検出 + 全 landmarks あり | あり | あり | あり | あり |
| hand 検出あるが quality not good または landmark 欠損 | なし | なし | あり | あり |
 ...
 *     timestampMs = undefined, normalizedThumbDistance = undefined,
 *     geometrySignature = undefined, sourceKey = undefined
 *
 * - detection があり worldLandmarks が不在:
 *     handDetected = true, sideViewQuality = detection.sideViewQuality,
 *     normalizedThumbDistance = undefined, geometrySignature = undefined
 *
 * - detection と worldLandmarks がある:
 *     normalizedThumbDistance = dist(thumbTip, indexMcp) / max(0.0001, dist(wrist, indexMcp))
 *     geometrySignature = { wristToIndexMcp, wristToMiddleMcp, indexMcpToPinkyMcp }
 *     sideViewQuality はそのまま forward
 *
...
3. **p10 / p90 nearest-rank 計算**：
   ...
   - 外れ値 5 個を含む 90 サンプルで p10/p90 が 8 番目/82 番目の sorted 値と一致
4. **clamp 動作**：
...
- 入力：`iterations/telemetry-2026-04-19T01-18-36-449Z.json`
- 各フレームの side detection から `extractSideTriggerRawMetric` → adaptive reducer → `extractSideTriggerEvidence` → 既存 FSM の simulation
- 期待値：**commit 数 ≥ 18**（22 ジェスチャの 80% 以上）
- これは `package.json:15-19` の `check` から呼ばれる `test:replay` に組み込み、**CI 必須**にする（既存 replay infra に乗せる）

これにより、本設計が解決を約束した「20 ジェスチャの大半を発火」を回帰として保証する。
...
- 2026-04-19 初版
- 2026-04-19 改訂: codex review (commit 530c895) と codex advisor の指摘を反映
