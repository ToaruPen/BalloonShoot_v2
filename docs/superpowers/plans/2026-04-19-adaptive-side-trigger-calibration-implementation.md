...
**Goal:** Side-camera trigger を子どもや成人を含む幅広いユーザの可動域に自動追従させる runtime adaptive calibration を導入し、現地テストで未発火だった問題を解消する（Phase 1: replay gate `≥ 13` commits）。
...
**Source of truth:** `docs/superpowers/specs/2026-04-19-adaptive-side-trigger-calibration-design.md`（改訂 r6）を最終仕様とする。本プランと spec が衝突した場合は spec を優先し、本プランを後追い更新する。
...
- ローカル（fixture あり）：replay test 自体が pass する（adaptive >= 13）、baseline static < adaptive
...
- [ ] `npm run test:replay` の adaptive baseline が `>= 13 commits` を満たす
...
- [ ] PR 説明に：simulation 結果（旧 capture 0.05 で 16 commits / 0.40 で 9 commits、新 capture 0.05 で 20/25 commits / 0.40 で 5/25 commits）、root cause、hold guard 設計、Phase 2 で扱う tuning 軸、replay fixture の出所説明
...
- **risk:** Phase 1 ゲート ≥13 を実装が下回る（今回の simulation は Codex 環境で行ったため、TypeScript 実装で再現性が完全保証されているわけではない）
