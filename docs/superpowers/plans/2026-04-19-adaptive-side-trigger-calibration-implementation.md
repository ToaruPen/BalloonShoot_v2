...
**Goal:** Side-camera trigger を子どもや成人を含む幅広いユーザの可動域に自動追従させる runtime adaptive calibration を導入し、現地テストで未発火だった問題を解消する（Phase 1: replay gate `≥ 8` commits）。

**Architecture:** 既存 `createSideTriggerMapper` は無変更のまま、純粋 reducer + wrapper mapper を新設して calibration を side-channel observer から供給する。`pulled` と `open` の両方を sliding-window p10/p90 で適応し、release 評価との結合問題を回避する。

**Tech Stack:** TypeScript 6 + Vite + Vitest + MediaPipe HandLandmarker。pure functional FSM、ESLint boundaries、knip。

**Source of truth:** `docs/superpowers/specs/2026-04-19-adaptive-side-trigger-calibration-design.md` (commit `a06e52e`、改訂 r4) を最終仕様とする。本プランと spec が衝突した場合は spec を優先し、本プランを後追い更新する。
 ...
 */
export const INITIAL_SIDE_TRIGGER_OPEN_POSE_DISTANCE = 1.20;
```
...
```ts
import type { Point3D, SideHandDetection, SideViewQuality } from "../../shared/types/hand";
...
export const assertAdaptiveCalibrationConfig = (config: AdaptiveSideTriggerCalibrationConfig): void => {
...
7. **percentile 計算**（push があった場合のみ）：nearest-rank で p10 = `sorted[ceil(0.10*n)-1]`, p90 = `sorted[ceil(0.90*n)-1]`
...
2. **線形補間 (pulled)**：n=0 で INITIAL_PULLED 出力、n=15 で blend 50%、n=30+ で clampedP10
3. **線形補間 (open)**：同上の構造で INITIAL_OPEN → clampedP90
4. **p10/p90 nearest-rank**：
   - n=10、`[0.1, 0.2, ..., 1.0]` → p10=0.1 (sorted[0])、p90=0.9 (sorted[8])
   - n=90、ceil(0.10*90)-1=8 → 9 番目の sorted 値、ceil(0.90*90)-1=80 → 81 番目
5. **clamp 規則 4-step**：
   - 5a. p10 < 0 → clamp to 0
   - 5b. p90 > 1.2 → clamp to 1.2
   - 5c. clampedOpen - clampedPulled < 0.4 → 中点保持で押し広げ
   - 5d. shift 結果が境界超過 → 反対端へシフト
   - 5e. 退化 config (`openUpperBound - pulledLowerBound < pulledOpenMinSpan`) → assertConfig で early throw
...
9. **quality gate 5 ケース** (spec 8a-8e に厳密対応、各 7 副作用すべて assert)：
   - 8a: good + thumb + signature あり：push, ema更新, sample+1, p10/p90 更新, ema更新, ts更新, timer評価
   - 8b: good + thumb + signature 欠損：push, ema更新なし, sample+1, p10/p90 更新, ema不変, ts更新, timer評価。連続 100 フレームで hand-loss reset しないことを assert
   - 8c: 非 good (`tooOccluded`) + thumb あり：push なし, sample 不変, p10/p90 不変, ema不変, ts更新, timer評価
...
11. **`assertAdaptiveCalibrationConfig` テスト**：spec の各 invariant violation を 1 件ずつ throw 検証
12. **fixedOpen 不変 (=1.2 cap)**：8a-8e 全ケースで `output.open <= 1.2`

**Dependencies**

- Task 2 (INITIAL_* constants)
...
```ts
import { createSideTriggerMapper, type SideTriggerMapper, type SideTriggerMapperUpdate, type SideTriggerMapperResult } from "./createSideTriggerMapper";
import {
...
export const createAdaptiveSideTriggerMapper = (
  ...
  return {
    update(update) {
      ...
      adaptiveState = updateSideTriggerAdaptiveCalibration(adaptiveState, metric, config);
      return inner.update({
...
### Task 10: Replay test (CI 必須ゲート)

**Scope:** `~≤2.5h`

**Files**

- Create: `tests/fixtures/replay/sideTriggerAdaptive/baseline-2026-04-19.json` — `iterations/telemetry-2026-04-19T01-18-36-449Z.json` から複製
- Create: `tests/replay/sideTriggerAdaptiveCalibration.replay.test.ts`
- Modify: `tests/replay/AGENTS.md` — recorded-but-deterministic JSON fixture を許容する旨を追記

**Implementation**

- `iterations/telemetry-2026-04-19T01-18-36-449Z.json` を `tests/fixtures/replay/sideTriggerAdaptive/baseline-2026-04-19.json` に **無変更で複製**（git に追加）
- replay test：fixture を読み込み、各 frame の `side` detection を `extractSideTriggerRawMetric` → `updateSideTriggerAdaptiveCalibration` → `extractSideTriggerEvidence` → 既存 `updateSideTriggerState` (FSM) の pipeline に流す
- 比較対象として **同じ fixture を static calibration (`defaultSideTriggerCalibration`) で流した baseline** も同テスト内で計算
- 期待値を `expect(adaptiveCommits).toBeGreaterThanOrEqual(8)` で hard gate
...
`tests/replay/AGENTS.md` に以下を追記：

```
## Notes on captured-but-deterministic fixtures

`tests/fixtures/replay/sideTriggerAdaptive/baseline-2026-04-19.json` は
MediaPipe HandLandmarker が出力した landmark JSON のスナップショット。
動画ファイル (.mov 等) と異なり完全に決定論的なテキストであり、
合成入力と同様に扱う。録画動画の復活ではない。
```

**Test plan**

- replay test 自体が pass する（adaptive >= 8）
- baseline static < adaptive の不変条件
...
- 2 コミットを推奨：
  - `chore(test): add captured side-trigger telemetry fixture for replay`（fixture 複製のみ）
  - `test(replay): gate adaptive calibration regression at >=8 commits`（test 本体）
...
- [ ] `npm run test:replay` の adaptive baseline が `>= 8 commits` を満たす
...
- [ ] PR 説明に：簡単な改善グラフ (static 1 commit → adaptive ≥ 8 commits)、Phase 2 で扱う tuning 軸へのリンク、replay fixture の出所説明
...
- **risk:** Phase 1 ゲート ≥8 を実装が下回る（今回の simulation は Codex 環境で行ったため、TypeScript 実装で再現性が完全保証されているわけではない）
  - **mitigation:** Task 5 完了直後（Task 6 以降に進む前）に、replay test を一度回して数値を確認。下回る場合は config defaults を仕様の Phase 2 軸（windowSamples 短縮、p10/p90 → p15/p85 等）で軽く tuning できる余地を残しているので、その範囲で対処
